...
plot(elev)
plot(st_geometry(survey_big), add = T)
...
Because it's just a large both, so not very precise 
...
# Plot prominence
plot(prominence)
# the legend reflect which cells are visible from any given location, so 80 means that that point is visible 80 % of the time from any given location
...
elevation_prom_overlay <- overlay(prominence, elevation2, fun = f)


# Plot the result (low elevation and high prominence areas)
plot(elevation_prom_overlay)
# for some reason this isn't showing the correct image 
```
...
# finding mounds with high visibility
...
# find the 25 most visible mounds 
mounds_25 <- mounds %>% dplyr::arrange(desc(prominence)) %>% dplyr::slice(1:25)

# finding the trap codes with unique
